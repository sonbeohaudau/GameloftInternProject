...
void MemoryManager::Dump()
{
	...
	for(unsigned int i = 0; i < m_noBuffers; i++)
	{
		//esLogMessage("%4d. 0x%08X: %d bytes(%s: %d)\n", i, (unsigned long)m_dataBuffers[i].pAddress, m_dataBuffers[i].length,m_dataBuffers[i].fileName, m_dataBuffers[i].line);
...
void MemoryManager::SanityCheck(bool bShowStats)
{
	...
	for(unsigned int i = 0; i < m_noBuffers; i++)
	{
		...
		{
			//esLogMessage("memory corruption at 0x%08X: %d bytes(%s: %d)\n", (unsigned long)m_dataBuffers[i].pAddress, m_dataBuffers[i].length, m_dataBuffers[i].fileName, m_dataBuffers[i].line);
			count++;
		}
	}
